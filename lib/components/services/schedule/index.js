...
const ruleToUTCDateTime = require('./utils/rule-to-datetime')
const localDateTimeToUTCRule = require('./utils/datetime-to-rule')
...
class CronService {
  ...
  async startTask (key) {
    ...
    if (scheduleType === 'interval') {
      ...
      const rule = localDateTimeToUTCRule(datetime)
    ...
    const email = modifiedBy ?? createdBy
    const userId = email ? await this.userInfo.userIdFromEmail(email) : null

    // todo: allow window for start/end datetimes
    task.job = nodeSchedule.scheduleJob(
      jobOptions,
      () => {
        console.log(`Running scheduled state machine ${stateMachineName} at ${new Date()}`)
        ...
        if (userId) {
          executionOptions.userId = userId
        }
  ...
  async findTasks (blueprintTasks = {}) {
    ...
    for (const [key, config] of Object.entries(blueprintTasks)) {
      ...
      if (typeof config.interval === 'string') {
        ...
        config.datetime = ruleToUTCDateTime(config.rule)
  ...
  persistTask (key, config) {
    ...
    if (config.createdBy) {
      task.createdBy = config.createdBy
    }
  ...
  async stopAndUpdateTaskSchedule (key, rule) {
    ...
    if (typeof rule === 'string') {
      ...
      this.scheduledTasks[key].datetime = ruleToUTCDateTime(rule)
